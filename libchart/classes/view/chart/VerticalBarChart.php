            ...
            for ($j = 0; $j < $serieCount; $j++) {
                ...
                for ($i = 0; $i < $pointCount; $i++) {
                    ...
                    if ($this->config->getShowPointCaption()) 
                    {
                        ...
                        //mlm $x1 -4 à cause de l'affichage des stats avec 3 chiffres pour les heures
                        $text->printText($img, $x1 + ($barWidth / 2) -4, $ymin - 5, $this->plot->getTextColor(), $value, $text->fontCondensed, $text->HORIZONTAL_CENTER_ALIGN | $text->VERTICAL_BOTTOM_ALIGN, $fontsize);
                    }
                    ...
                    if ($ymin != $graphArea->y2) {
                        ...
                        imagefilledrectangle($img, $x1 + 1, $ymin + 1, $x2 - 4, $graphArea->y2 - 1, $point->getColor());
